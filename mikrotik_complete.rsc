...
# 8. Sync Scripts
/system script
add name=SyncUsers source=":global apiUrl; :global apiToken; :local syncedUsers \"\"; :do { /tool fetch url=(\$apiUrl . \"?action=getNewUsers&token=\" . \$apiToken) mode=http check-certificate=no keep-result=yes dst-path=users.txt; :local content [/file get users.txt contents]; :if ([:len \$content] > 0) do={ :local pos 0; :while (\$pos < [:len \$content]) do={ :local end [:find \$content \"|\" \$pos]; :if ([:typeof \$end] = \"nil\") do={ :set end [:len \$content] }; :local record [:pick \$content \$pos \$end]; :set pos (\$end + 1); :if ([:len \$record] > 0) do={ :local fields [:toarray \$record]; :local uname (\$fields->0); :local pass (\$fields->1); :local duration (\$fields->2); :local speed (\$fields->3); :if ([:len [/ip hotspot user find name=\$uname]] = 0) do={ /ip hotspot user add name=\$uname password=\$pass limit-uptime=\$duration rate-limit=\$speed comment=\"Synced\"; :log info \"Added user: \$uname\"; } else={ /ip hotspot user set [find name=\$uname] password=\$pass limit-uptime=\$duration rate-limit=\$speed; }; :set syncedUsers (\$syncedUsers . \$uname . \",\"); }; }; :if ([:len \$syncedUsers] > 0) do={ /tool fetch url=(\$apiUrl . \"?action=markSynced&token=\" . \$apiToken . \"&usernames=\" . \$syncedUsers) check-certificate=no keep-result=no; }; }; } on-error={ :log error \"Sync failed\" }"

add name=KickUsers source=":global apiUrl; :global apiToken; :do { /tool fetch url=(\$apiUrl . \"?action=getKickList&token=\" . \$apiToken) mode=http check-certificate=no keep-result=yes dst-path=kick.txt; :local content [/file get kick.txt contents]; :if ([:len \$content] > 0) do={ :local pos 0; :while (\$pos < [:len \$content]) do={ :local end [:find \$content \"|\" \$pos]; :if ([:typeof \$end] = \"nil\") do={ :set end [:len \$content] }; :local uname [:pick \$content \$pos \$end]; :set pos (\$end + 1); :if ([:len \$uname] > 0) do={ /ip hotspot user remove [find name=\$uname]; /ip hotspot active remove [find user=\$uname]; :log info \"Removed user: \$uname\"; }; }; }; } on-error={ :log error \"Kick failed\" }"
